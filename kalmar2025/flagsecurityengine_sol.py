...
resett=False
flag = ['a']*88
flag[0:len("kalmar{")]=list("kalmar{")

def find_diff_pos(s1, s2):
...
class CustomBreakpoint(gdb.Breakpoint):
    ...
    def do_sth(self):
        global check,varr,resett
        addr1 = 0x555555673440
        addr2 = 0x5555556732c0
        
        str1 = gdb.execute(f"x/b {addr1+varr}", to_string=True).split(":")[1].strip()
        str2 = gdb.execute(f"x/b {addr2+varr}", to_string=True).split(":")[1].strip()

        print(f"String1: {str1}, String2: {str2}" )  # Debug output
        if str1!=str2:
            check += 1
...
def set_breakpoint(address):
    CustomBreakpoint(address)  

def run_program():
    global check,varr,resett
    trc=""
    while(True):
        char_list = list(string.ascii_lowercase + string.ascii_uppercase + string.digits + "-!}{_$^=")
        for j in char_list:
            flag[varr]=j
            check=0
            code=f"""// load the Kalmar Flag Security Engine™
...
checkFlag('{''.join(flag)}')
"""
            newfile = open("bff.js","w")
            newfile.write(code)
            #print(code)
            gdb.execute(f"run bff.js")
...
address_to_breakpoint = "0x55555559e6ac"
set_breakpoint(address_to_breakpoint)  
run_program()  
